...
func TestServerNewRejectsInvalidTransportParameters(t *testing.T) {
	...
	_, err := server.New(cfg, testLogger())
	if err == nil {
...
func testServerConfig(certFile, keyFile string) server.Config {
...
func newIntegrationServerTLSConfig(t *testing.T, serverName string) *tls.Config {
	certFile, keyFile := writeTestCertificateFiles(t)
...
func newIntegrationClientTLSConfig(t *testing.T, serverName string) *tls.Config {
	cfg := newIntegrationServerTLSConfig(t, serverName)
