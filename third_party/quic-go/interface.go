...
type Connection interface {
	...
	// The path is identified by the given PathID and sends packets to the given remote address.
	// It is a no-op if the connection was not configured with a PathSelector.
	...
	UpdatePathRSSI(id PathID, rssi int) error
}
...
type Config struct {
	...
	// InitialMaxPathID advertises experimental draft-21 multipath support when non-zero.
