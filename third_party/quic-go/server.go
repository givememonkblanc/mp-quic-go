...
func (s *baseServer) handlePacket(p receivedPacket) {
	fmt.Printf("[handlePacket] from %s, len=%d\n", p.remoteAddr, p.Size())
	// Always log for debugging
	if s.logger != nil {
		s.logger.Debugf("[handlePacket] from %s, len=%d", p.remoteAddr, p.Size())
	}
	select {
	case s.receivedPackets <- p:
		fmt.Printf("[handlePacket] sent to receivedPackets queue\n")
	default:
		fmt.Printf("[handlePacket] dropping packet - queue full\n")
...
func (s *baseServer) handlePacketImpl(p receivedPacket) bool /* is the buffer still in use? */ {
	fmt.Printf("[handlePacketImpl] from %s, len=%d, type=%d\n", p.remoteAddr, len(p.data), p.data[0])
