...
type connection struct {
	...
	pathHandlers  map[PathID]*PathHandler
...
var newConnection = func(
	...
	if conf.PathSelector != nil {
		s.SetPathSelector(conf.PathSelector)
	}
	return s
}
...
// SetPathSelector sets the path selection strategy for multipath QUIC.
// If nil, the connection behaves as a regular single-path QUIC connection.
func (s *connection) SetPathSelector(ps PathSelector) {
	s.pathSelector = ps
}

// AddPathHandler registers an additional path for multipath sending.
// The path's send queue is started and will be used by the send loop.
func (s *connection) AddPathHandler(h *PathHandler) {
	s.pathHandlers[h.ID] = h
	if h.SendQ != nil && h.SendQ.conn != nil {
		s.pathByAddr[h.SendQ.conn.RemoteAddr().String()] = h
	}
	go func() {
		if err := h.RunSendQueue(); err != nil {
			s.logger.Errorf("path %d send queue error: %v", h.ID, err)
		}
	}()
}

// AddPath adds an additional path for multipath QUIC.
// It creates a new PathHandler with its own packet number space and send queue,
// using the underlying rawConn to send to the given remote address.
// This is a no-op if the connection was not configured with a PathSelector.
func (s *connection) AddPath(addr net.Addr, id PathID) error {
	if id == 0 {
	...
	if s.mainRawConn == nil {
		return fmt.Errorf("main rawConn not available")
	}
...
// RemovePathHandler removes and closes a path handler.
func (s *connection) RemovePathHandler(id PathID) {
	if h, ok := s.pathHandlers[id]; ok {
		h.Close()
		delete(s.pathHandlers, id)
	}
}
...
// UpdatePathRSSI updates the RSSI value tracked for the given path.
// The RSSI is used by the PathSelector for informed path selection decisions.
func (s *connection) UpdatePathRSSI(id PathID, rssi int) error {
	s.pathRSSIsLock.Lock()
	defer s.pathRSSIsLock.Unlock()
	if s.pathRSSIs == nil {
		s.pathRSSIs = make(map[PathID]int)
	}
	s.pathRSSIs[id] = rssi
	return nil
}
...
func (s *connection) maybeResetTimer() {
	...
	alarmTimeout := s.receivedPacketHandler.GetAlarmTimeout()
	for _, ph := range s.pathHandlers {
		if ph.RecvPH == nil {
			continue
		}
		if t := ph.RecvPH.GetAlarmTimeout(); !t.IsZero() && (alarmTimeout.IsZero() || t.Before(alarmTimeout)) {
			alarmTimeout = t
		}
	}
	lossTimeout := s.sentPacketHandler.GetLossDetectionTimeout()
	for _, ph := range s.pathHandlers {
		if ph.SentPH == nil {
			continue
		}
		if t := ph.SentPH.GetLossDetectionTimeout(); !t.IsZero() && (lossTimeout.IsZero() || t.Before(lossTimeout)) {
			lossTimeout = t
		}
	}
	s.timer.SetTimer(
...
func (s *connection) handleShortHeaderPacket(p receivedPacket) bool {
	...
	if err := s.handleUnpackedShortHeaderPacket(recvPH, sentPH, destConnID, pn, data, p.ecn, p.rcvTime, log); err != nil {
		s.closeLocal(err)
		return false
	}
	return true
...
func (s *connection) handleLongHeaderPacket(p receivedPacket, hdr *wire.Header) bool /* was the packet successfully processed */ {
	...
	// Long header packets (Initial, Handshake, 0-RTT) use the main path's handler
	// since additional paths are established only after handshake.
	...
	if err := s.handleUnpackedLongHeaderPacket(s.receivedPacketHandler, packet, p.ecn, p.rcvTime, p.Size()); err != nil {
		s.closeLocal(err)
		return false
	}
	return true
...
func (s *connection) handleUnpackedLongHeaderPacket(
	...
	isAckEliciting, err := s.handleFrames(packet.data, packet.hdr.DestConnectionID, packet.encryptionLevel, log, s.sentPacketHandler)
	if err != nil {
		return err
	}
	return recvPH.ReceivedPacket(packet.hdr.PacketNumber, ecn, packet.encryptionLevel, rcvTime, isAckEliciting)
}
...
func (s *connection) handleUnpackedShortHeaderPacket(
	...
	isAckEliciting, err := s.handleFrames(data, destConnID, protocol.Encryption1RTT, log, sentPH)
	if err != nil {
		return err
	}
	return recvPH.ReceivedPacket(pn, ecn, protocol.Encryption1RTT, rcvTime, isAckEliciting)
}
...
func (s *connection) handleFrames(
	...
	for len(data) > 0 {
		...
		if err := s.handleFrame(frame, encLevel, destConnID, sentPH); err != nil {
			if log == nil {
				return false, err
			}
...
func (s *connection) handleFrame(f wire.Frame, encLevel protocol.EncryptionLevel, destConnID protocol.ConnectionID, sentPH ackhandler.SentPacketHandler) error {
	var err error
	...
	case *wire.AckFrame:
		err = s.handleAckFrame(frame, encLevel, sentPH)
...
func (s *connection) handleAckFrame(frame *wire.AckFrame, encLevel protocol.EncryptionLevel, sentPH ackhandler.SentPacketHandler) error {
	acked1RTTPacket, err := sentPH.ReceivedAck(frame, encLevel, s.lastPacketReceivedTime)
	if err != nil {
		return err
	}
	if !acked1RTTPacket {
	...
	// Per-path packet numbers are independent — extra paths start from 0.
	// Only update the global largest-1-RTT-acked from the main path to avoid
	// overwriting with a smaller packet number from an extra path's PN space.
	if s.perspective == protocol.PerspectiveClient && !s.handshakeConfirmed {
		if err := s.handleHandshakeConfirmed(); err != nil {
			return err
		}
	}
	if sentPH == s.sentPacketHandler {
...
func (s *connection) handleCloseError(closeErr *closeError) {
	...
	// Close all path handler send queues to stop their goroutines.
	for _, ph := range s.pathHandlers {
		ph.Close()
	}
...
func (s *connection) dropEncryptionLevel(encLevel protocol.EncryptionLevel) error {
	...
	for _, ph := range s.pathHandlers {
		ph.SentPH.DropPackets(encLevel)
		ph.RecvPH.DropPackets(encLevel)
	}
...
func (s *connection) sendPackets(now time.Time) error {
	...
	if err := s.sendOnExtraPaths(now); err != nil {
		return err
	}
...
// sendOnExtraPaths sends packets on extra paths.
...
func (s *connection) sendOnExtraPaths(now time.Time) error {
	...
	if s.pathSelector != nil {
		// Use the PathSelector to pick one path for this send cycle.
		states := s.pathStates()
		selectedID := s.pathSelector.SelectPath(states)
		if selectedID != 0 {
			if ph, ok := s.pathHandlers[selectedID]; ok {
				if !ph.SendQ.WouldBlock() {
					return s.sendOnPath(ph, now)
				}
			}
		}
		// If selector returned 0 (main path) or an unknown path ID,
		// skip extra paths this cycle.
		return nil
	}

	// Fallback: send on all extra paths.
	for _, ph := range s.pathHandlers {
		if ph.SendQ.WouldBlock() {
			continue
		}
