...
func (p *TransportParameters) readNumericTransportParameter(b []byte, paramID transportParameterID, expectedLen int) error {
	...
	case initialMaxPathIDParameterID:
		if val > 0xffffffff {
			return fmt.Errorf("initial_max_path_id too large: %d", val)
		}
		p.InitialMaxPathID = uint32(val)
...
func (p *TransportParameters) ValidFor0RTT(saved *TransportParameters) bool {
	...
	return p.InitialMaxStreamDataBidiLocal >= saved.InitialMaxStreamDataBidiLocal &&
		...
		p.InitialMaxPathID >= saved.InitialMaxPathID
}
...
func (p *TransportParameters) String() string {
	...
	logParams = append(logParams, []interface{}{p.InitialMaxStreamDataBidiLocal, p.InitialMaxStreamDataBidiRemote, p.InitialMaxStreamDataUni, p.InitialMaxData, p.MaxBidiStreamNum, p.MaxUniStreamNum, p.MaxIdleTimeout, p.AckDelayExponent, p.MaxAckDelay, p.ActiveConnectionIDLimit, p.InitialMaxPathID}...)
