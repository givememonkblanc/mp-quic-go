...
func TestUpdatableAEADPathAwareNonceChangesCiphertext(t *testing.T) {
	client, server, _ := setupEndpoints(t, &utils.RTTStats{})
	plaintext := []byte("Lorem ipsum")
	ad := []byte("Donec in velit neque.")

	path0 := server.SealForPath(nil, plaintext, 0, 0x1337, ad)
	path1 := server.SealForPath(nil, plaintext, 1, 0x1337, ad)
	require.NotEqual(t, path0, path1)

	opened, err := client.OpenForPath(nil, path1, time.Now(), 1, 0x1337, protocol.KeyPhaseZero, ad)
	require.NoError(t, err)
	require.Equal(t, plaintext, opened)

	_, err = client.OpenForPath(nil, path1, time.Now(), 0, 0x1337, protocol.KeyPhaseZero, ad)
	require.Equal(t, ErrDecryptionFailed, err)
}

func TestUpdatableAEADDecodePacketNumberPerPath(t *testing.T) {
	client, server, _ := setupEndpoints(t, &utils.RTTStats{})
	plaintext := []byte("Lorem ipsum")
	ad := []byte("Donec in velit neque.")

	encrypted0 := server.SealForPath(nil, plaintext, 0, 0x1337, ad)
	_, err := client.OpenForPath(nil, encrypted0, time.Now(), 0, 0x1337, protocol.KeyPhaseZero, ad)
	require.NoError(t, err)
	require.Equal(t, protocol.PacketNumber(0x1338), client.DecodePacketNumberForPath(0, 0x38, protocol.PacketNumberLen1))
	require.Equal(t, protocol.PacketNumber(0x38), client.DecodePacketNumberForPath(1, 0x38, protocol.PacketNumberLen1))

	encrypted1 := server.SealForPath(nil, plaintext, 1, 0x42, ad)
	_, err = client.OpenForPath(nil, encrypted1, time.Now(), 1, 0x42, protocol.KeyPhaseZero, ad)
	require.NoError(t, err)
	require.Equal(t, protocol.PacketNumber(0x43), client.DecodePacketNumberForPath(1, 0x43, protocol.PacketNumberLen1))
}
