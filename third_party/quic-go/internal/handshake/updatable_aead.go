...
type updatableAEAD struct {
	...
	nonceBuf   []byte
	pathStates map[uint32]*pathAEADState
}

type pathAEADState struct {
	highestRcvdPN  protocol.PacketNumber
	firstSentPN    protocol.PacketNumber
	firstRecvPN    protocol.PacketNumber
	lastOpenedPath uint32
}
...
func newUpdatableAEAD(rttStats *utils.RTTStats, tracer *logging.ConnectionTracer, logger utils.Logger, version protocol.Version) *updatableAEAD {
	return &updatableAEAD{
		...
		logger:                  logger,
		version:                 version,
		pathStates:              make(map[uint32]*pathAEADState),
	}
}
...
func (a *updatableAEAD) DecodePacketNumber(wirePN protocol.PacketNumber, wirePNLen protocol.PacketNumberLen) protocol.PacketNumber {
	return a.DecodePacketNumberForPath(0, wirePN, wirePNLen)
}

func (a *updatableAEAD) DecodePacketNumberForPath(pathID uint32, wirePN protocol.PacketNumber, wirePNLen protocol.PacketNumberLen) protocol.PacketNumber {
	state := a.getPathState(pathID)
	return protocol.DecodePacketNumber(wirePNLen, state.highestRcvdPN, wirePN)
}
...
func (a *updatableAEAD) Open(dst, src []byte, rcvTime time.Time, pn protocol.PacketNumber, kp protocol.KeyPhaseBit, ad []byte) ([]byte, error) {
	return a.OpenForPath(dst, src, rcvTime, 0, pn, kp, ad)
}

func (a *updatableAEAD) OpenForPath(dst, src []byte, rcvTime time.Time, pathID uint32, pn protocol.PacketNumber, kp protocol.KeyPhaseBit, ad []byte) ([]byte, error) {
	dec, err := a.open(dst, src, rcvTime, pathID, pn, kp, ad)
	...
	if err == nil {
		state := a.getPathState(pathID)
		state.highestRcvdPN = max(state.highestRcvdPN, pn)
		if state.firstRecvPN == protocol.InvalidPacketNumber {
			state.firstRecvPN = pn
		}
		state.lastOpenedPath = pathID
		a.highestRcvdPN = max(a.highestRcvdPN, pn)
	}
	return dec, err
}

func (a *updatableAEAD) open(dst, src []byte, rcvTime time.Time, pathID uint32, pn protocol.PacketNumber, kp protocol.KeyPhaseBit, ad []byte) ([]byte, error) {
...
func (a *updatableAEAD) Seal(dst, src []byte, pn protocol.PacketNumber, ad []byte) []byte {
	return a.SealForPath(dst, src, 0, pn, ad)
}

func (a *updatableAEAD) SealForPath(dst, src []byte, pathID uint32, pn protocol.PacketNumber, ad []byte) []byte {
	...
	state := a.getPathState(pathID)
	if state.firstSentPN == protocol.InvalidPacketNumber {
		state.firstSentPN = pn
	}
...
func (a *updatableAEAD) getPathState(pathID uint32) *pathAEADState {
	state, ok := a.pathStates[pathID]
	if ok {
		return state
	}
	state = &pathAEADState{highestRcvdPN: protocol.InvalidPacketNumber, firstSentPN: protocol.InvalidPacketNumber, firstRecvPN: protocol.InvalidPacketNumber}
	a.pathStates[pathID] = state
	return state
}

func (a *updatableAEAD) buildNonce(pathID uint32, pn protocol.PacketNumber) {
	for i := range a.nonceBuf {
		a.nonceBuf[i] = 0
	}
	if len(a.nonceBuf) >= 12 {
		offset := len(a.nonceBuf) - 12
		binary.BigEndian.PutUint32(a.nonceBuf[offset:offset+4], pathID)
		binary.BigEndian.PutUint64(a.nonceBuf[len(a.nonceBuf)-8:], uint64(pn)&0x3fffffffffffffff)
		return
	}
